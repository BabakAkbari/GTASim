...
Run FiveM.exe click on the `localhost` to connect to the server in the developer mode or simply press `F8`. This will open up your client console then type `connect loalhost:30120` to connect to your server. After the game is loaded go back to your server console and execute `start GTASim` to run GTASim scripts.
